const prizes = [
  { label: "1000", icon: "fa-coins", color: "#ff9f43", textColor: "#fff" },         // HIGHLIGHT (Sunset Orange)
  { label: "SURPRESA", icon: "fa-gift", color: "#ff6b6b", textColor: "#fff" },      // COLOR A (Coral)
  { label: "PONTOS", icon: "fa-star", color: "#20b2aa", textColor: "#fff" },        // COLOR B (Ocean)
  { label: "50", icon: "fa-gem", color: "#ff6b6b", textColor: "#fff" },             // COLOR A
  { label: "BRINDE", icon: "fa-gift", color: "#20b2aa", textColor: "#fff" },        // COLOR B
  { label: "100", icon: "fa-trophy", color: "#ff6b6b", textColor: "#fff" },         // COLOR A
  { label: "BÔNUS", icon: "fa-dice", color: "#20b2aa", textColor: "#fff" },         // COLOR B
  { label: "500MM", icon: "fa-award", color: "#ff6b6b", textColor: "#fff" }         // COLOR A
];
...
function initWheel() {
  ...
  prizes.forEach((prize, index) => {
    ...
    gradientParts.push(`${prize.color} ${start}deg ${end}deg`);
...
function startIdle() { if (isSpinning) return; document.body.classList.add('is-idling'); }
function stopIdle() {
...
function tickPointer() { if (!pointer) return; pointer.classList.remove('tick'); void pointer.offsetWidth; pointer.classList.add('tick'); }
...
function startTickEffect(duration) {
  ...
  tickInterval = setInterval(() => {
    ...
    if (progress >= 1) { clearInterval(tickInterval); tickInterval = null; }
  }, 30);
}
...
function spin() {
  ...
  startSlowSpin();
  fetchPrizeFromServer().then((winnerIndex) => { stopSlowSpin(); executeSpinToWinner(winnerIndex); }).catch(() => { stopSlowSpin(); isSpinning = false; btnGirar.disabled = false; if (centerText) centerText.textContent = "GIRAR"; startIdle(); });
}

function startSlowSpin() {
  let slowRotation = currentRotation;
  function animate() { slowRotation += 1.5; wheelFrame.style.transition = 'none'; wheelFrame.style.transform = `rotate(${slowRotation}deg)`; slowSpinAnimation = requestAnimationFrame(animate); }
  animate();
}
...
function fetchPrizeFromServer() {
  return new Promise((resolve) => { setTimeout(() => { resolve(Math.floor(Math.random() * prizes.length)); }, 1000 + Math.random() * 1000); });
}
...
function showCelebration(prizeLabel) {
  createConfetti();
  setTimeout(() => { if (prizeValue) prizeValue.textContent = prizeLabel; if (prizePopup) prizePopup.classList.add('show'); }, 300);
}

function createConfetti() {
  if (!celebrationOverlay) return;
  const colors = ['#ff9f43', '#ff6b6b', '#20b2aa', '#f4e4bc', '#87ceeb'];
