const prizes = [
  { label: "1000", icon: "fa-coins", color: "#d4af37", textColor: "#fff" },         // HIGHLIGHT (Gold)
  { label: "SURPRESA", icon: "fa-gift", color: "#c41e3a", textColor: "#fff" },      // COLOR A (Red)
  { label: "PONTOS", icon: "fa-star", color: "#1a1a1a", textColor: "#fff" },        // COLOR B (Black)
  { label: "50", icon: "fa-gem", color: "#c41e3a", textColor: "#fff" },             // COLOR A (Red)
  { label: "BRINDE", icon: "fa-gift", color: "#1a1a1a", textColor: "#fff" },        // COLOR B (Black)
  { label: "100", icon: "fa-trophy", color: "#c41e3a", textColor: "#fff" },         // COLOR A (Red)
  { label: "BÔNUS", icon: "fa-dice", color: "#1a1a1a", textColor: "#fff" },         // COLOR B (Black)
  { label: "500MM", icon: "fa-award", color: "#c41e3a", textColor: "#fff" }         // COLOR A (Red)
];
...
function createLights() {
  ...
  for (let i = 0; i < totalLights; i++) {
    ...
    light.style.animationDelay = `${i % 2 === 0 ? '0s' : '0.4s'}`;
...
function createSectorElement(prize, index, sectorAngle) {
  ...
  content.style.color = prize.textColor || "white";
...
function executeSpinToWinner(winnerIndex) {
  ...
  setTimeout(() => {
    ...
    btnGirar.disabled = false;
    const centerText = btnGirar.querySelector('.center-text');
    if (centerText) centerText.textContent = "SPIN";
    btnGirar.removeAttribute('aria-busy');
  }, SPIN_DURATION_MS);
}
...
function createConfetti() {
  ...
  const colors = ['#d4af37', '#c41e3a', '#fff', '#ffd700', '#ff6b6b', '#4ecdc4'];
